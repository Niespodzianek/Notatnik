...
def operacja_na_pliku(typ_dzialania):
    ...
    try:
        if typ_dzialania == "odczyt":
            ...
            zapis_do_pliku(
                sciezka=sciezka_do_pliku, tekst_do_zapisu=tekst_notatki()
            )
        ...
        print(
            f"Nie udało się odczytać pliku, błąd typu - {type(error)} - {error}"
        )
    else:
...
def program_notatnik():
    while True:
        ...
        if wybor_menu == "1":
            ...
            input(
                "Koniec pracy programu. Naciśnij ENTER aby zakończyć program."
            )
            exit()
        else:
